...
if($_GET['action']=='save'){
	...
	header('Content-Disposition: attachment; filename="รายงานตาม-Leadder.xls"');#ชื่อไฟล์
...
<table width="95%" border="1" align="center" cellpadding="0" cellspacing="0" class='table table-bordered tablesorter'>
  <thead>
  <tr>
    <td align="center" style="font-size:18px; text-align:center;"> รายงานตาม Leadder</td>
  </tr>
  </thead>
</table>
<?php
if($_POST['report_id']=="All"){
		$sql="select t1.*,Count(t2.leader) as Total_Row from leader t1,people t2 Where t1.auto_id=t2.leader  GROUP BY t2.leader Order By Total_Row DESC";
}else{
		//$sql="select * from leader where auto_id='".$_POST['report_id']."' Order By auto_id ASC";
		$sql="select t1.*,Count(t2.leader) as Total_Row from leader t1,people t2 Where t1.auto_id=t2.leader and  t1.auto_id='".$_POST['report_id']."'  GROUP BY t2.leader Order By Total_Row DESC";
}
//echo $sql;
...
<thead>
<tr>
  <td height="30" colspan="9" align='left' style="font-weight:bold;"><?php echo $data['leader_name'] ;?> </td>
  </tr>
<tr>
